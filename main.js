...
if (!product) {
      throw new Error(`Produit avec l'ID ${stock.id} introuvable`);
    }

  const changeStock = (stock.type === 'IN' ? 1 : -1) * stock.amount

  if (product.stock_available + changeStock < 0) throw new Error('negative stock')

  const data = {
    stock_available: product.stock_available + changeStock,
  }
